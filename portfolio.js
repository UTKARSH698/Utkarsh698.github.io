...
(function () {
  ...
  function run(raw) {
    ...
    if (c === "clear" || c === "cls") { state.termLines = []; renderTerm(); return; }
      ...
      out.push({ text: "aws-samples/well-architected #104  \u2713 merged — pillar playbooks in full-review dispatch", color: G });
      out.push({ text: "Lynpoint/CyberVerse #32       \u2713 merged — pluggable RAG vector store (Milvus)", color: G });
  ...
  function layoutResume() {
    ...
    para("Under review: prowler #11839 (Glue connection secrets check), aws-cdk #38195 (account-root ARN in resource policies), opentelemetry-python-contrib #4728 (Redis ClusterPipeline spans).", 9.6, 0.2); y += 6;
